...
fn main() -> anyhow::Result<()> {
  ...
  match args.command {
    ...
    Commands::MakeJS => {
      ...
      println!("=> removing {} and {} exports", "initSync".bold(), "__wbg_init".bold());
      let js = js
      ...
      println!("=> removing {} old property", "__wbg_init.__wbindgen_wasm_module".bold());
      ...
      println!("=> rewriting module instanciation in {} function", "initSync".bold());
      let js = js.replace(
      ...
      println!("=> wasm base64url length: {}", wasm.len().to_string().yellow());
      ...
      let types = regex.replace_all(&types, |caps: &Captures| {
        ...
        println!("=> replacing '{}' with '{}'", &caps[0].red(), &replacement.green());
        replacement
      }).to_string();
      ...
      println!("=> types file path: {}", types_file_path.to_str().unwrap().underline());
